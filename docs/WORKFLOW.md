...
The image query is generated at the same time as each slide. This matters
because Claude understands the meaning of the slide while it is writing the
query. Selecting images later does not require another Claude call.
...
better with scenes that could physically appear in a photograph.
...
When a pasted script is composed locally, there is no AI-generated query. The
browser creates a lightweight fallback by removing punctuation and common
words, deduplicating the remaining words, and keeping up to four terms. A
manually created slide can also fall back to its hook text when opening image
search.
